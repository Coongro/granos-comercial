# granos-comercial - Development Guide
...
- **Trigger name**: `trg_granos_comercial_{descriptive_name}`
- **Function name**: `trg_granos_comercial_{descriptive_name}_fn`

Where `granos_comercial` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_granos_comercial_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_granos_comercial_audit_log"
AFTER INSERT OR UPDATE ON "module_granos_comercial_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_granos_comercial_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_granos_comercial_*` triggers when the plugin
